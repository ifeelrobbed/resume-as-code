...
APP_DIR := app
MANIFEST_DIRS := manifests promql
BIN_DIR := .bin
...
.PHONY: help fmt vet build test app-check yamllint kubeconform manifests-check \
...
yamllint: ## Lint manifests/ and promql/ with yamllint
	...
	yamllint -d "$(YAMLLINT_CONFIG)" $(MANIFEST_DIRS)
...
manifests-check: yamllint kubeconform ## Run all manifests/promql checks (yamllint, kubeconform)
